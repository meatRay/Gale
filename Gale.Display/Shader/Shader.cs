...
namespace Gale
{
	public partial class Shader : IDisposable
	{
		...
		public int MusicLocation { get; private set; } = -1;
		...
		private static Shader CompileFromShaders(IEnumerable<Step> shader_steps)
		{
			...
			int music_id = GL.GetUniformLocation(id, "music");
			return new Shader(id, model_id, tex_id, pos_id, uv_id, proj_id, z_id, view_id, music_id);
		}
